...
public class UMLClassDraw {
    ...
    private UMLRootClassDrawOld rootClassDraw;
    ...
    public UMLClassDraw(UMLClassObject classObject, UMLRootClassDrawOld rootClassDraw, FontMetrics fontMetrics){
    ...
    private Point[] GetRelationLinePath(UMLClassDraw to){
        if(classObject.IsInterface()) {
            UMLRootClassDrawOld[] rootClassPath = to.GetRootClassesPath();
    ...
    private Point[] ConvertRootClassPathToPath(UMLRootClassDrawOld[] rootClassPath){
    ...
    public UMLRootClassDrawOld[] GetRootClassesPath(){
        List<UMLRootClassDrawOld> path = new ArrayList<>();
        path.add(rootClassDraw);
        while(path.get(path.size() - 1).GetParentDraw() != null){
            path.add(path.get(path.size() - 1).GetParentDraw());
        }
        return path.toArray(new UMLRootClassDrawOld[]{});
    }
